...
void DatagramServer<StaticConfig>::worker_proc(uint16_t lcore_id) {
  ...
  size_t next_index = 0;
  while (!stopping_) {
    //ra.check_host_dma_buf_for_requests();
...
//      begin_times.push_back(now);
      ...
      ra.setup(&rx_tx_state, count, now, lcore_id);
      /*if (enable_printf) {
	      in_batch++;
...
void DatagramServer<StaticConfig>::RequestAccessor::
    ...
    doca_error_t res_2 = doca_dma_write((uint16_t)::mica::util::lcore.lcore_id());
