...
class DatagramServer {
 ...
 private:
  ...
  // Doca
  doca_error_t doca_init(uint16_t lcore_id);
  ...
  doca_error_t save_config_info_to_files(char *export_desc, size_t export_desc_len, const char *src_buffer, size_t src_buffer_len, char *export_desc_file_path, char *buffer_info_file_path);
  doca_error_t doca_init_recv_buf(uint16_t lcore_id);
  uint8_t check_dma_recv_buf_for_response(uint16_t lcore_id);
  ...
  void check_pending_tx_full_2(RXTXState& tx_state);
  ...
  class RequestAccessor : public ::mica::processor::RequestAccessorInterface {
   public:
    ...
    Operation get_operation_from_dpu_dma_send_buf(size_t index, uint16_t lcore_id);
    uint64_t get_key_hash(size_t index);
    uint64_t get_key_hash_from_dpu_dma_send_buf(size_t index, uint16_t lcore_id);
    ...
    void parse_requests_from_dpdk(uint16_t lcore_id, uint8_t pkt_count);
    doca_error_t doca_dma_write(uint16_t lcore_id);
    void prepare_response_batch(uint16_t lcore_id, uint8_t pkt_count);
    void print_response_in_dpu_dma_recv_buf(uint8_t count);
    void process_response_from_host(uint16_t lcore_id);
    void flush_partial_pending_response_batch(const PacketBuffer* src_buf);

    void dummy_parse(uint16_t lcore_id, uint16_t pkt_count);
