...
class RequestBatchReader {
 public:
  ...
  uint64_t get_timestamp() const { return h_->timestamp; }
...
class RequestBatchBuilder {
 public:
  ...
  void append_request_no_key_value(Operation operation, Result result,
                                   ...
                                   size_t key_length, size_t value_length, uint64_t timestamp) {
  ...
  void append_request_base(Operation operation, uint32_t opaque, uint64_t key_hash,
		  	   size_t key_length, size_t value_length, uint64_t timestamp) {
    h_->operation = static_cast<uint8_t>(operation);
    h_->key_hash = key_hash;
    h_->opaque = opaque;
  ...
  void append_result(PacketBuffer *pb, uint8_t result) {
    RequestHeader* hh = reinterpret_cast<RequestHeader*>(pb->get_data() + sizeof(RequestBatchHeader));
    hh->result = result;
  }

  void append_value(PacketBuffer *pb, uint64_t value) {
    RequestHeader* hh = reinterpret_cast<RequestHeader*>(pb->get_data() + sizeof(RequestBatchHeader));
    auto p = reinterpret_cast<char*>(hh) + sizeof(RequestHeader);
    ::mica::util::memcpy<8>(p, &value, 8);
  }

  void append_request(Operation operation, Result result, uint32_t opaque,
                      uint64_t key_hash, const char* key, size_t key_length,
                      const char* value, size_t value_length, uint64_t timestamp) {
    assert(has_sufficient_space(key_length, value_length));
    assert(count_ != 0xff);
    assert(key_length < (1 << 8));
    assert(value_length < (1 << 24));
