...
void DPDK<StaticConfig>::init_mempool() {
  ...
  for (uint16_t numa_id = 0; numa_id < StaticConfig::kMaxNUMACount; numa_id++) {
    ...
    */
	...
	pool_name, static_cast<unsigned int>(mbuf_size),
	static_cast<unsigned int>(cache_size), RTE_MBUF_PRIV_ALIGN,
	(uint16_t)2176, 0);
...
/*  if (rx_packets) {
	  double u_sec = ((end.tv_sec - start.tv_sec) * 1000000000.0 + (end.tv_nsec - start.tv_nsec)) / 1000;
	  printf("$$$ lcore %2zu received %" PRIu16 " packets (rte_eth_rx_burst) on port_id %" PRIu16 " and queue_id %" PRIu16 " in time %.2f\n",
                  ::mica::util::lcore.lcore_id(), rx_packets, port_id, queue_id, u_sec);
  }*/
...
uint16_t DPDK<StaticConfig>::send(EndpointId eid, PacketBuffer** bufs,
  ...
  /*if (tx_packets) {
	  double u_sec = ((end.tv_sec - start.tv_sec) * 1000000000.0 + (end.tv_nsec - start.tv_nsec)) / 1000;
	  printf("$$$ lcore %2zu transmitted %" PRIu16 " packets (rte_eth_tx_burst) on port_id %" PRIu16 " and queue_id %" PRIu16 " in time %.2f\n",
                  ::mica::util::lcore.lcore_id(), tx_packets, port_id, queue_id, u_sec);
  }*/
